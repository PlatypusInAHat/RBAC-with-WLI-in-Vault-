...
kubectl get namespaces | Select-String -Pattern "prj|vault"

Write-Host ""
Write-Host "=== Pods in prj namespace ===" -ForegroundColor Yellow
...
Write-Host "  1. Port forward frontend: kubectl port-forward -n prj svc/frontend 30080:3000"
...
Write-Host "  kubectl get sa -n prj"
Write-Host ""
Write-Host "  # View backend SA details"
Write-Host "  kubectl describe sa prj-backend-sa -n prj"
Write-Host ""
Write-Host "  # Check roles & bindings"
Write-Host "  kubectl get roles,rolebindings -n prj"
Write-Host ""
Write-Host "  # Test from inside pod"
Write-Host '  $POD = kubectl get pod -n prj -l component=backend -o jsonpath="{.items[0].metadata.name}"'
Write-Host '  kubectl exec -n prj $POD -- cat /var/run/secrets/kubernetes.io/serviceaccount/namespace'
Write-Host ""
...
Write-Host '  $POD = kubectl get pod -n prj -l component=backend -o jsonpath="{.items[0].metadata.name}"'
Write-Host '  kubectl describe pod -n prj $POD | Select-String "vault"'
Write-Host ""
Write-Host "  # Check if secrets are injected"
Write-Host '  kubectl exec -n prj $POD -c backend -- ls -la /vault/secrets/'
Write-Host ""

Write-Host "🔄 TEST 4: Secret Sharing (Backend ↔ Frontend)" -ForegroundColor Green
Write-Host "  # Port forward backend"
Write-Host "  kubectl port-forward -n prj svc/backend 30000:3000"
Write-Host ""
...
Write-Host '  $BE_POD = kubectl get pod -n prj -l component=backend -o jsonpath="{.items[0].metadata.name}"'
Write-Host '  $FE_POD = kubectl get pod -n prj -l component=frontend -o jsonpath="{.items[0].metadata.name}"'
Write-Host '  kubectl exec -n prj $BE_POD -c backend -- cat /vault/secrets/shared'
Write-Host '  kubectl exec -n prj $FE_POD -c frontend -- cat /vault/secrets/shared'
Write-Host ""

Write-Host "🌐 TEST 5: Network Policies" -ForegroundColor Green
Write-Host "  # Check network policies"
Write-Host "  kubectl get networkpolicies -n prj"
Write-Host ""
Write-Host "  # Test connectivity from frontend to backend (should work)"
Write-Host '  $FE_POD = kubectl get pod -n prj -l component=frontend -o jsonpath="{.items[0].metadata.name}"'
Write-Host '  kubectl exec -n prj $FE_POD -- curl -s http://backend-internal:3000/health'
Write-Host ""
Write-Host "  # Test backend to database (should work)"
Write-Host '  $BE_POD = kubectl get pod -n prj -l component=backend -o jsonpath="{.items[0].metadata.name}"'
Write-Host '  kubectl exec -n prj $BE_POD -- nc -zv postgres 5432'
Write-Host ""
...
Write-Host "  kubectl get pods -n prj -w"
Write-Host "  kubectl logs -n prj -l component=backend --tail=50 -f"
Write-Host "  kubectl describe pod -n prj <pod-name>"
